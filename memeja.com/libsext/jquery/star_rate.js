...
/*# AVOID COLLISIONS #*/
	...
	$.rating = {
		...
		//callback: function(){alert(this.value);},  // executed when a star is clicked
	...
	$.fn.rating = function(instanceSettings){
		...
		this.each(function(i){
			...
			eStar = $('<div class="star"><a title="'+this.title+ '" name="'+this.value+'">&nbsp;</a></div>');
			$(this).after(eStar);
...
/*# AVOID COLLISIONS #*/
	...
	function showrating(source_div, start, end, url, target_div, initial){
		makeelement = "";
		for (i=start; i<=end; ++i) {
			makeelement += "<input name='"+source_div+"' value='"+source_div+i+"' title='"+i+"' type='radio' class='star'/>";
		}
		document.getElementById(source_div).innerHTML = makeelement;
		for (i=start; i<end; ++i) {
			if(i==initial){
				$('input[title="'+initial+'"]').attr('checked','checked');
			}
		}
		$(function(){
			...
			var value;
			var xx;
			$('#'+source_div+'> div.star a').attr({style:'background:url("'+url+'") no-repeat 0 0px;'});
			$('#'+source_div+'> div.star_live a').mouseover(function(){
				title=parseInt(this.title);
				//alert(title);
					for(i=1;i<=title;i++){
						$('#'+source_div+'>div.star_live a[name="'+source_div+i+'"]').css({backgroundPosition:'0 -32px'});
					}
				}).mouseout(function(){
					for(i=1;i<=title;i++){
						$('#'+source_div+'>div.star_live a[name="'+source_div+i+'"]').css({backgroundPosition:'0 0px'});
					}
				}).click(function (){
					//$('#'+target_div).html(title);
					document.getElementById(target_div).value=title;
						//alert(title);
			});
