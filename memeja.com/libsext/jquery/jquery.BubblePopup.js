...
(function ($) {
	...
	$.fn.SetBubblePopup = function (options) {
		var _ = {	objTags: ['object', 'embed', 'applet'],
					...
					markup: '\
							...
							<tr> \
								...
								<td style="background-image:url({IMAGE_7}); background-repeat:repeat-x; text-align:{TAIL_ALIGN};"></td> \
		...
		options = $.extend({ 
			...
			color: 'manipulated',
			imageFolder: '../templates/bp_images',
			hideTail: false,
			hideObjectID: [],
			contentStyle: '',
			zIndex: 100,
		...
		function getMarkup(input_html) {
			...
			var m = _.markup.replace('{HTML_CONTENT}',input_html).replace('{TAIL_ALIGN}',( tailAlign=='left' || tailAlign=='left' ? ( tailAlign=='left' ? 'left' : 'right' ) : 'left' ));
			if(hideTail){ 
			...
			m = m.replace('{CONTENT_STYLE}','color:#000000;'+(contentStyle.indexOf('font-family:')>-1 ? '' : 'font-family:\'Trebuchet MS\',Arial;')+(contentStyle.indexOf('background-color:')>-1 ? '' : 'background-color:#FFF7CA;')+contentStyle);
			for (var i = 1; i < 9; i++) {
		...
		function openBubble(popup_obj, tag_obj) {
			...
			if (_.loading[i] || _.loaded[i]){
				...
				coord["left"] = (bubbleAlign=='left' || bubbleAlign=='right' ? ( bubbleAlign=='left' ? parseInt(f.left)-16 : parseInt(tag_obj.width())+parseInt(f.left)-parseInt(popup_obj.width())+18 ) : ( parseInt(f.left)+((parseInt(tag_obj.width())-parseInt(popup_obj.width())+18)/2)) );
				popup_obj.css({
					top: coord["top"],
					left: coord["left"],
					position: 'absolute',
