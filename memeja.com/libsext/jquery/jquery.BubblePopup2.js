...
(function ($) {
	...
	$.fn.SetBubblePopup = function (options) {
		var _ = {	objTags: ['object', 'embed', 'applet'],
					...
					me: this,
					markup: '\
							<table width="{TABLE_WIDTH}" cellpadding="0" cellspacing="0" style="{IE_FILTER_SHADOW}display:none; z-index:{Z-INDEX}; position:absolute; border:0px; border-collapse:collapse; margin-left:-75px;"> \
							<tbody> \
							<tr> \
		...
		options = $.extend({ 
			...
			color: 'green',
			imageFolder: '/shippers/templates/default/images/bp_images',
			hideTail: false,
			hideObjectID: [],
			contentStyle: '',
			zIndex: 100,
		...
		function getMarkup(input_html) {
			...
			var m = _.markup.replace('{HTML_CONTENT}',input_html).replace('{TAIL_ALIGN}',( tailAlign=='center' || tailAlign=='right' ? ( tailAlign=='left' ? 'left' : 'right' ) : 'center' ));
			if(hideTail){ 
