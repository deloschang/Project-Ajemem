...
class leaderboard_manager extends mod_manager {
	...
	function _lb_duels($q='0'){
		...
		if($this->_input['uname']!=''){
			...
			$sql_duels="SET @i=0;SELECT *,POSITION FROM (SELECT *, @i:=@i+1 AS POSITION FROM ".TABLE_PREFIX."user ORDER BY duels_won DESC ) t WHERE id_user=$id_user";    
	...
	function _lb_exp_point($q='0'){
		...
		if($this->_input['uname']!=''){
		    ...
		    // print $sql;
		     ...
		     $id_user=$res[0]['id_user'];
		     if($res){
			$sql_exp="SET @i=0;SELECT *,POSITION FROM (SELECT *, @i:=@i+1 AS POSITION FROM ".TABLE_PREFIX."user ORDER BY exp_point DESC ) t WHERE id_user=$id_user";    
	...
	function _lb_ques_won(){
	    ...
	    if($this->arg['chk']) {
		if($this->arg['iduser']!=''){
		    $sql_ques="SET @i=0;SELECT *,POSITION FROM (SELECT *, @i:=@i+1 AS POSITION FROM ".TABLE_PREFIX."user ORDER BY ques_week_won DESC ) t WHERE id_user='".$this->arg['iduser']."'";    
	...
	function _lb_achievements($q='0'){
	    ...
	    if($this->arg['chk']) {
		if($this->arg['iduser']!=''){
		    $sql_ach="SET @i=0;SELECT *,POSITION FROM (SELECT *, @i:=@i+1 AS POSITION FROM ".TABLE_PREFIX."user ORDER BY no_badges DESC ) t WHERE id_user='".$this->arg['iduser']."'";    
	...
	function _show_profile(){
	   ...
	   /* $res_frd=getsingleindexrow($sql_frd);
	    ...
	    $sql_duels="SET @i=0;SELECT *,POSITION FROM (SELECT *, @i:=@i+1 AS POSITION FROM ".TABLE_PREFIX."user ORDER BY duels_won DESC ) t WHERE id_user=".$this->_input['id'];
	    $sql_ach="SET @i=0;SELECT *,POSITION FROM (SELECT *, @i:=@i+1 AS POSITION FROM ".TABLE_PREFIX."user ORDER BY no_badges DESC ) t WHERE id_user=".$this->_input['id'];
	    $sql_exp="SET @i=0;SELECT *,POSITION FROM (SELECT *, @i:=@i+1 AS POSITION FROM ".TABLE_PREFIX."user ORDER BY exp_point DESC ) t WHERE id_user=".$this->_input['id'];
	    $sql_ques="SET @i=0;SELECT *,POSITION FROM (SELECT *, @i:=@i+1 AS POSITION FROM ".TABLE_PREFIX."user ORDER BY ques_week_won DESC ) t WHERE id_user=".$this->_input['id']; 
